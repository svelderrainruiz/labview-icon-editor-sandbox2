...
## ApplyVIPC.ps1
Applies a `.vipc` **VI Package Configuration** to a specific LabVIEW version and bitness using VIPM CLI. The script resolves version from `.lvversion` when `-LabVIEWVersion` is omitted and enforces a strict target contract: VIPC `TargetVersionNumeric` must exactly match `.lvversion` numeric version before `vipm install` executes.
