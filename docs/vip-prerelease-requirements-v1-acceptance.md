...
| A-010 | Core | VR-AST-001, VR-AST-004 | Build VIP and emit all required artifacts. | Publish prerelease and list release assets. | Assets include `.vip`, release-notes markdown, `gcli-logs` evidence, `vip-build-status` evidence, and Linux container packed library (`.lvlibp`). |
