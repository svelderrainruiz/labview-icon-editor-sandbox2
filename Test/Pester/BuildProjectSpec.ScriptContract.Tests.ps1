...
Describe 'BuildProjectSpec LabVIEWCLI contract' {
    BeforeAll {
        ...
        if (-not (Test-Path -Path $script:buildScript -PathType Leaf)) {
            throw "BuildProjectSpec.ps1 not found at $script:buildScript"
        }

        $script:pwshExe = (Get-Command pwsh -ErrorAction Stop).Source
    }

    function script:New-BuildProjectSpecFixtureRepo {
  ...
  <Item Name="My Computer" Type="My Computer">
    <Item Name="Build Specifications" Type="Build">
      ...
      <Item Name="Source Distribution" Type="Source Distribution">
        <Property Name="Destination[0].destName" Type="Str">source</Property>
      </Item>
    </Item>
  </Item>
</Project>
'@ | Set-Content -Path (Join-Path $fixtureRoot 'lv_icon_editor.lvproj') -Encoding ascii
    ...
    function script:Invoke-BuildProjectSpec {
        param(
            [Parameter(Mandatory = $true)]
            [string]$RepoRoot,
    ...
    It 'fails fast when LabVIEWCLI is missing from PATH' {
        $fixtureRoot = New-BuildProjectSpecFixtureRepo
        $emptyToolPath = Join-Path $fixtureRoot 'no-cli'
        New-Item -Path $emptyToolPath -ItemType Directory -Force | Out-Null

        $result = Invoke-BuildProjectSpec -RepoRoot $fixtureRoot -PathValue $emptyToolPath
...
'@ | Set-Content -Path (Join-Path $toolPath 'LabVIEWCLI.ps1') -Encoding ascii
        ...
        $result = Invoke-BuildProjectSpec `
...
'@ | Set-Content -Path (Join-Path $toolPath 'LabVIEWCLI.ps1') -Encoding ascii
        ...
        $pathValue = "{0};{1}" -f $toolPath, $env:PATH
        $result = Invoke-BuildProjectSpec -RepoRoot $fixtureRoot -PathValue $pathValue

        $result.ExitCode | Should -Not -Be 0
        ($result.Output -join "`n") | Should -Match 'Build specification output not found at expected path'
    }

    It 'fails fast for SourceDistribution when BuildSpecName is omitted' {
        $fixtureRoot = New-BuildProjectSpecFixtureRepo
        $toolPath = Join-Path $fixtureRoot 'toolbin'
        New-Item -Path $toolPath -ItemType Directory -Force | Out-Null
...
'@ | Set-Content -Path (Join-Path $toolPath 'LabVIEWCLI.ps1') -Encoding ascii
        ...
        $result = Invoke-BuildProjectSpec `
            -RepoRoot $fixtureRoot `
            -PathValue $pathValue `
            -AdditionalArgs @('-ProjectSpecType', 'SourceDistribution', '-OutputRelativePath', 'builds/source/source.zip')

        $result.ExitCode | Should -Not -Be 0
        ($result.Output -join "`n") | Should -Match 'BuildSpecName is required when ProjectSpecType=SourceDistribution'
    }

    It 'fails fast for SourceDistribution when OutputRelativePath is omitted' {
        $fixtureRoot = New-BuildProjectSpecFixtureRepo
        $toolPath = Join-Path $fixtureRoot 'toolbin'
        New-Item -Path $toolPath -ItemType Directory -Force | Out-Null
...
'@ | Set-Content -Path (Join-Path $toolPath 'LabVIEWCLI.ps1') -Encoding ascii
        ...
        $result = Invoke-BuildProjectSpec `
            -RepoRoot $fixtureRoot `
            -PathValue $pathValue `
            -AdditionalArgs @('-ProjectSpecType', 'SourceDistribution', '-BuildSpecName', 'Source Distribution')

        $result.ExitCode | Should -Not -Be 0
        ($result.Output -join "`n") | Should -Match 'OutputRelativePath is required when ProjectSpecType=SourceDistribution'
    }

    It 'runs SourceDistribution builds when spec name and output path are provided' {
        $fixtureRoot = New-BuildProjectSpecFixtureRepo
        $toolPath = Join-Path $fixtureRoot 'toolbin'
        New-Item -Path $toolPath -ItemType Directory -Force | Out-Null

        $argsCapturePath = Join-Path $fixtureRoot 'labviewcli-source-args.txt'
        $sourceOutputPath = Join-Path $fixtureRoot 'builds\source\source.zip'
        $sourceOutputDir = Split-Path -Path $sourceOutputPath -Parent
        New-Item -Path $sourceOutputDir -ItemType Directory -Force | Out-Null
...
$operation = $null
for ($index = 0; $index -lt $CliArgs.Count - 1; $index++) {
    if ($CliArgs[$index] -eq '-OperationName') {
        $operation = $CliArgs[$index + 1]
        break
    }
}

if ($operation -eq 'ExecuteBuildSpec' -and -not [string]::IsNullOrWhiteSpace($env:LVIE_TEST_SOURCE_OUTPUT_PATH)) {
    $dir = Split-Path -Path $env:LVIE_TEST_SOURCE_OUTPUT_PATH -Parent
    if (-not (Test-Path -Path $dir -PathType Container)) {
        New-Item -Path $dir -ItemType Directory -Force | Out-Null
    }
    'fake-source-output' | Set-Content -Path $env:LVIE_TEST_SOURCE_OUTPUT_PATH -Encoding ascii
}
...
'@ | Set-Content -Path (Join-Path $toolPath 'LabVIEWCLI.ps1') -Encoding ascii
        ...
        $envOverrides = @{
            LVIE_TEST_ARGS_PATH = $argsCapturePath
            LVIE_TEST_SOURCE_OUTPUT_PATH = $sourceOutputPath
        }
        ...
        $result = Invoke-BuildProjectSpec `
            ...
            -PathValue $pathValue `
            -Environment $envOverrides `
            -AdditionalArgs @('-ProjectSpecType', 'SourceDistribution', '-BuildSpecName', 'Source Distribution', '-OutputRelativePath', 'builds/source/source.zip')

        $result.ExitCode | Should -Be 0
        (Test-Path -Path $sourceOutputPath -PathType Leaf) | Should -BeTrue
        $capturedArgs = (Get-Content -Path $argsCapturePath -Raw)
        $capturedArgs | Should -Match '-OperationName ExecuteBuildSpec'
        $capturedArgs | Should -Match '-BuildSpecName Source Distribution'
        $capturedArgs | Should -Match '-TargetName My Computer'
    }
}
